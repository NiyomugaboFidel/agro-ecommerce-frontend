...
const FlashSaleItem = ({ item }) => {
  ...
  const { addToCart, removeFromCart, isInCart } = useCart(); // Destructure necessary functions from useCart
  const [quantity, setQuantity] = useState(item.moq);
  useEffect(() => {
    // No need to directly modify item.quantity; manage cart items via context
  }, [item]);
